-- ====================================================
-- أضف اللاب 5 وفلاج FLAG{UNPROTECTED_ADMIN_PANEL}
...
SELECT 5, 'ACCESS_CONTROL_BYPASS', 'Test role-based access control: bypass restrictions and escalate privileges', 3, 'medium', 100, @creator, 1, 'public', 'cyberops/access-control-lab', 3600
FROM (SELECT 1) AS _tmp
WHERE NOT EXISTS (SELECT 1 FROM labs WHERE lab_id = 5);

-- التحدي للاب 5 (إن لم يكن موجوداً)
INSERT IGNORE INTO challenges (challenge_id, lab_id, created_by, title, statement, order_index, max_score, difficulty, is_active)
SELECT 6, 5, @creator, 'UNPROTECTED_ADMIN_PANEL', 'Access the admin panel without authorization', 1, 50, 'medium', 1
FROM (SELECT 1) AS _tmp
WHERE NOT EXISTS (SELECT 1 FROM challenges WHERE lab_id = 5);
...
-- تأكد أن أي testcase مرتبط بالاب 5 يحمل الفلاج الصحيح
UPDATE testcases t
