...
import { WHITEBOX_SQL_LAB_ID, WHITEBOX_SQL_EXTRA_LAB_IDS } from "../../constants/labs";
...
const LabsListModern = ({
  ...
  const filteredLabs = labs.filter((lab) => {
    ...
    if (labTypeId != null) {
      if (labTypeId === 1) {
        ...
        if (lab.labtype_id !== 1 && id !== WHITEBOX_SQL_LAB_ID && !WHITEBOX_SQL_EXTRA_LAB_IDS.includes(id)) return false;
      } else if (labTypeId === 2) {
  ...
  const handleOpenLab = (lab) => {
    if (onLabClick) {
      ...
      const path = [WHITEBOX_SQL_LAB_ID, ...WHITEBOX_SQL_EXTRA_LAB_IDS, 18, 19].includes(wid) ? "/lab-whitebox" : "/lab-modern";
